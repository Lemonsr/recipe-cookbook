...
function infixToPostfix(ops) {
  ...
  for (let i = 0; i < ops.length; i++) {
    ...
    if (op in operations) { // if operator is scanned, pop all operators that are higher in precedence before it
      while (stack.length != 0 && prec(stack[i]) <= prec(stack[stack.length - 1])) {
        postfix.push(stack.pop());
        stack.pop();
      }
      stack.push(op); // then add the operator to stack
    } else { // if its an operand, add to result stack
